 ...
 */
function defaultMarkerSize(shape: MarkerShape | undefined): number {
  return shape === VLMarkerShapeEnum.ARROW ? DEFAULT_ARROW_MARKER_SIZE : DEFAULT_MARKER_SIZE
}

export function resolveMarkerConfig(
  input: MarkerShape | MarkerConfig | undefined,
  ...
  return {
    shape,
    size: config.size ?? defaultMarkerSize(shape),
...
function builtinShapeMarkup(shape: MarkerShape, color: string): string {
  const fill = `fill="${color}"`
  switch (shape) {
    case VLMarkerShapeEnum.CIRCLE:
      return `<circle cx="${CENTER}" cy="${CENTER}" r="${CIRCLE_MARKER_RADIUS}" ${fill} />`
    case VLMarkerShapeEnum.SQUARE: {
      const size = MARKER_VIEWBOX - SQUARE_MARKER_INSET * 2
      return `<rect x="${SQUARE_MARKER_INSET}" y="${SQUARE_MARKER_INSET}" width="${size}" height="${size}" ${fill} />`
    }
    case VLMarkerShapeEnum.DIAMOND: {
      const far = MARKER_VIEWBOX - DIAMOND_MARKER_INSET
      return `<polygon points="${CENTER},${DIAMOND_MARKER_INSET} ${far},${CENTER} ${CENTER},${far} ${DIAMOND_MARKER_INSET},${CENTER}" ${fill} />`
    }
...
export function createMarkerElement(id: string, position: 'start' | 'end', marker: ResolvedMarker): SVGMarkerElement {
  ...
  el.innerHTML = marker.svg ?? builtinShapeMarkup(marker.shape ?? VLMarkerShapeEnum.CIRCLE, marker.color)
  return el
}
